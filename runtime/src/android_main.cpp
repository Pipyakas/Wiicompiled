...
Java_org_patchzyy_wiicompiled_GameActivity_nativeGetGxDiagnostics(JNIEnv* env, jobject) {
    ...
    uint64_t cDse = g_sceneChainCallCounts.dvdStoreErr.load(std::memory_order_relaxed);
    uint64_t cDec = g_sceneChainCallCounts.dvdErrCb.load(std::memory_order_relaxed);
    uint64_t cDrd = g_sceneChainCallCounts.dvdReady.load(std::memory_order_relaxed);
    uint64_t cDcs = g_sceneChainCallCounts.dvdCmdStatus.load(std::memory_order_relaxed);
    uint64_t cDth = g_sceneChainCallCounts.dvdThread.load(std::memory_order_relaxed);
    uint64_t cRip = g_sceneChainCallCounts.ripImpl.load(std::memory_order_relaxed);
    uint64_t cRp2 = g_sceneChainCallCounts.rip.load(std::memory_order_relaxed);
    uint64_t cMB = g_sceneChainCallCounts.mainB6B0.load(std::memory_order_relaxed);
    uint64_t cMR = g_sceneChainCallCounts.mainRK.load(std::memory_order_relaxed);
    uint64_t cSC = g_sceneChainCallCounts.sysCt.load(std::memory_order_relaxed);
    uint64_t cRI = g_sceneChainCallCounts.rkInit.load(std::memory_order_relaxed);
    uint64_t cTI = g_sceneChainCallCounts.tsysInit.load(std::memory_order_relaxed);
    uint64_t cDC = g_sceneChainCallCounts.discCreate.load(std::memory_order_relaxed);
    uint64_t cSH = g_sceneChainCallCounts.sysHeap.load(std::memory_order_relaxed);
    uint64_t cLM = g_sceneChainCallCounts.loadMap.load(std::memory_order_relaxed);
    uint64_t cRT = g_sceneChainCallCounts.ripTask.load(std::memory_order_relaxed);
    uint64_t cRO = g_sceneChainCallCounts.runObj.load(std::memory_order_relaxed);
    uint64_t cTQ = g_sceneChainCallCounts.taskReq.load(std::memory_order_relaxed);
    ...
    uint64_t cRL = g_sceneChainCallCounts.ripLoad.load(std::memory_order_relaxed);
    uint64_t cRA = g_sceneChainCallCounts.ripA.load(std::memory_order_relaxed);
    uint64_t cRB = g_sceneChainCallCounts.ripB.load(std::memory_order_relaxed);
    uint64_t cRC = g_sceneChainCallCounts.ripC.load(std::memory_order_relaxed);
    uint64_t cRD = g_sceneChainCallCounts.ripD.load(std::memory_order_relaxed);
    ...
    std::snprintf(buf, sizeof(buf), "gxbeg=%llu gxend=%llu gxdl=%llu gxfifo=%llu dlbeg=%llu dlend=%llu dlact=%llu"
        ...
        " r3=0x%llx r21=0x%llx vt=0x%llx dvd=%llu dth=%llu dse=%llu dec=%llu drd=%llu dcs=%llu"
        " boot[rip=%llu rp2=%llu mb=%llu mr=%llu sc=%llu ri=%llu ti=%llu dc=%llu"
        " sh=%llu lm=%llu rt=%llu runObj=0x%llx tq=%llu te=%llu su=%llu re=%llu ec=%llu dm=%llu rl=%llu"
        " ra=%llu rb=%llu rc=%llu rd=%llu dr=%llu dg=%llu os=%llu]] %s",
        ...
        (unsigned long long)cDvd, (unsigned long long)cDth,
        (unsigned long long)cDse, (unsigned long long)cDec,
        (unsigned long long)cDrd, (unsigned long long)cDcs,
        (unsigned long long)cRip, (unsigned long long)cRp2, (unsigned long long)cMB,
        (unsigned long long)cMR, (unsigned long long)cSC, (unsigned long long)cRI,
        ...
        (unsigned long long)cEC, (unsigned long long)cDM,
        (unsigned long long)cRL,
        (unsigned long long)cRA, (unsigned long long)cRB,
        (unsigned long long)cRC, (unsigned long long)cRD,
        (unsigned long long)cDR, (unsigned long long)cDG,
        (unsigned long long)cOS, rbuf);
