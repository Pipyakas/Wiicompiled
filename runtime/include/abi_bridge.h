...
struct SceneChainCallCounts {
    ...
    // RipImpl file-load step: DvdRipper::LoadToMainRAM per file.
    std::atomic<uint64_t> ripLoad{0};    // 0x802226D8
    std::atomic<uint64_t> ripA{0};       // 0x8022235C LoadToMainRAM sub-step
...
inline void ApplyRuntimeCallOptions(uint32_t target, CpuContext* ctx) {
    ...
    case 0x802226D8u: g_sceneChainCallCounts.ripLoad.fetch_add(1, std::memory_order_relaxed); break;
    case 0x8022235Cu: g_sceneChainCallCounts.ripA.fetch_add(1, std::memory_order_relaxed); break;
...
inline void CountIndirectVtableSlot(uint32_t target, CpuContext* cpu) {
    // Temporary (see indTotal above): indirect calls only. Static
    // MKW_STATIC_TRANSLATED_CALL bodies never pass through here; their
